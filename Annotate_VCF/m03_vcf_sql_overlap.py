...
# get fsgs columns
cursor = conn.execute('select * from fsgs limit 2')
...
new_fsgs_col = ['fsgs.' + i for i in fsgs_col]
new_varcards_col = []
...
cursor.execute(cmd)
cmd = ('SELECT {c} FROM fsgs \
        LEFT JOIN varcards \
        ON fsgs.chr = varcards.Chr AND \
        fsgs.pos = varcards.Start AND \
        fsgs.ref = varcards.Ref AND \
        fsgs.alt = varcards.Alt').format(
        c=','.join(all_columns))
cursor.execute(cmd)
